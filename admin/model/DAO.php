...
{
    $sql = "INSERT INTO `attribute` (`ten`) VALUES (?)";
    pdo_execute($sql, $name);
}

function update_attribute($id, $name)
{
    $sql = "UPDATE `attribute` SET `ten` = ? WHERE `id` = ?";
...
{
    $sql = "INSERT INTO `brand` (`ten`, `mo_ta`) VALUES (?, ?)";
...
{
    $sql = "UPDATE `brand` SET `ten` = ?, `mo_ta` = ? WHERE `id` = ?";
...
function create_danh_muc($name, $description)
{
    $sql = "INSERT INTO `danh_muc` (`ten_danh_muc`, `mo_ta`) VALUES (?, ?)";
    pdo_execute($sql, $name, $description);
}

function update_danh_muc($id, $name, $description)
{
...
function get_product_by_danh_muc($id) {
    $sql = "SELECT * FROM `product` WHERE `danh_muc_id` = ?";
    return pdo_query_one($sql, $id);
}
...
FROM
    product p
LEFT JOIN product_variant_danh_muc pvc ON
    p.id = pvc.product_id
LEFT JOIN product_variant pv ON
    pvc.id = pv.variant_danh_muc_id
GROUP BY
    p.id
ORDER BY
    p.id;
...
function get_variant_danh_muc($id) {
    $sql = "SELECT * FROM `product_variant_danh_muc` WHERE product_id = $id;";
    return pdo_query($sql);
}

function add_product_variant_danh_muc($product_id, $variant_type, $description) {
    $sql = "INSERT INTO `product_variant_danh_muc` (`product_id`, `variant_type`, `desc`) VALUES (?, ?, ?);";
...
function delete_product_variant_danh_muc($id) {
